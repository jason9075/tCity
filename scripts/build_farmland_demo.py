...
def build():
    ...
    tcity.register();obj=make_farmland(width=360,depth=260) if village else make_farmland();mod=farmland_modifier(obj)
    ...
    if village:aerial.location=(250,-365,365);aim(aerial,Vector((0,2,0)))
    top=camera('02 • Parcel plan / 田區配置',(0,-.01,400),(0,0,0),50);top.data.type='ORTHO';top.data.ortho_scale=270
    if village:top.data.ortho_scale=390
    ...
    homes=[(i.object.original.get('tc_farm_asset'),i.matrix_world.copy()) for i in bpy.context.evaluated_depsgraph_get().object_instances
           if i.is_instance and i.parent and i.parent.original==obj and i.object.original.get('tc_farm_asset') in (6,8,9,12,13,14)]
    home=next((m for k,m in homes if k==(12 if village else 8)),homes[0][1] if homes else None)
    rural=camera('04 • Rural home / 農舍與鄉間環境',home@Vector((17,-23,12)) if home else (40,-70,20),home@Vector((0,0,2)) if home else (0,0,1),43)
    if village and home is not None:rural.location=home@Vector((29,-40,24));aim(rural,home@Vector((0,0,2)))
    ...
    if village:scene.view_settings.exposure=.65
    ...
    intro.from_string(intro.as_string().replace('Farmland v0.2','Farmland v0.3'))
    ...
    if village:
        intro.clear();intro.write('TCity · 農村聚落 / Farmland v0.3\n\nN 側欄 > TCity > 台灣農地 > 農村聚落。\n沿路聚落密度 / Village Mix 控制聚落範圍內的住宅院落機率。\n房屋朝向最近農路；紅瓦平房、兩層農舍、鐵皮工作棚與前院組成住宅群。\n建議田寬至少 38 m、田長至少 44 m，太小的院落會因安全間距而不放房屋。\nStructures 可隱藏房屋；Village Mix = 0 還原田地用途；Farm Roads 關閉時也取消聚落用地。\n四個相機：空拍、田區配置、田埂近景、農村住宅。\n這是參考照片特徵製作的程序化場景，並非原村莊的精確重建。\n')
    dest=ROOT/('dist/TCity_Taiwan_Village.blend' if village else 'dist/TCity_Taiwan_Farmland.blend');dest.parent.mkdir(exist_ok=True)
    ...
    if '--render' in sys.argv:
        ...
        for cam,name in [(aerial,'farmland_aerial'),(top,'farmland_plan'),(close,'farmland_detail'),(rural,'farmland_rural')]:
            if village:name=name.replace('farmland','village')
