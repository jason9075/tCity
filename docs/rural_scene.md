...
灰模構圖確認後，以相同 JSON、基地與院落配置接入農村組件。範例包含 49 戶主屋、33 個側屋、33 塊田地、8,779 個作物實例及 92 根電桿，並有 61 跨架空線（每跨四條）、23 段明渠（約 1,606 m）、14 處安全田界轉角接合、4 處地下箱涵（約 25.6 m）及 8 座接入井。住宅的 82 個可見量體、92 根電桿、244 條架空線與 23 段明渠現在由 Geometry Nodes 實例／曲線層生成。

```sh
blender -b --factory-startup --python-exit-code 1 --python scripts/build_rural_blockout.py -- --assets --render
```

- `dist/TCity_Rural_Settlement.blend`：包含來源分區、道路 Curve、住宅／公用設施／明渠 Geometry Nodes、作物實例及五部相機。
...
明渠的規劃結果輸入 `Rural irrigation channels / GN paths`，每一條 spline 對應一段通過道路、住宅、入口、電桿與邊界檢查的田界渠道。`Rural channels / Geometry Nodes` 以 Curve to Mesh 分別掃出槽底、兩側混凝土壁與水面；轉角接合及涵管修剪後的 station 範圍會重新寫入這些 spline。原始槽體 mesh 保留為隱藏的幾何驗證來源，箱涵與接入井仍使用明確 mesh 以保留道路缺口與地表開口。
...
已加入架空線與田界明渠。架空線連接同一道路上相鄰的既有電桿，三條上層線與一條下層線的兩端對準絕緣子／支架，預設垂度 0.55 m、最大跨距 40 m；過長或投影跨越住宅／入口的線段略過，不跨道路編號自動接網。244 條線材路徑輸入 `Rural overhead wires / GN paths`，由 Curve to Mesh 生成可見線材；原始逐線 mesh 保留為隱藏驗證來源。明渠則以 23 條田界 spline 由同一個 GN host 掃出開放槽體。`build_rural_scene` 可用 `cable_sag`（0–1.5 m）與 `max_wire_span`（5–60 m）調整；目前是視覺生成參數，非電力工程設計。
...
架空線與明渠路徑屬於重建結果，修改道路／電桿／田界後需重新生成；生成後可直接編輯 GN 點與曲線觀察結果。尚無院牆、共用院落與前後排住宅。田地平面仍位於道路下方，作物已避路，但尚未做道路範圍的精確田地差集。建築細節以中遠景為主，尚非照片級重建。
