...
`tc_layer` 現有語意（道路 1、人行道 2、架空線 3）維持，新增 4 = 非道路地面。
...
- 由道路網切分街廓多邊形（`Fill Curve` + 布林），街廓內再切基地，取代「沿線兩排」的簡化。
- 殘餘空地轉停車場／綠地（roadmap 已列）。
- 曲率過大時的基地略過規則與 UI 警示。
...
| 曲率過大時基地仍可能不合理 | 視覺破圖 | 0.5 先輸出警示屬性，0.7 做自動略過 |
