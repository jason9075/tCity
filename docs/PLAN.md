...
- **已完成（0.7 開發版）— 區域屬性控制**：基地中心從來源 Mesh 表面取樣 `tc_zone_vacancy`、`tc_zone_height`、`tc_zone_min_floors`、`tc_zone_max_floors`、`tc_zone_facade_mix`、`tc_zone_commercial`、`tc_zone_era`。0–1 的 vertex group 適合 Weight Paint 空置、固定樓高、立面／商業／年代比例；`tc_zone_height` 映射為固定 2–7 層並優先於供 face attribute 使用的絕對 min/max。缺少屬性時回退 0、既有全域控制或原始隨機資產分布。商業與年代依現有六種立面中實際有店面／招牌、老舊粉刷／磚面、較新磁磚／馬賽克的特徵分組，不宣稱為尚未製作的大型商場或歷史建築資產。
