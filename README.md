...
目前 checkout 的城市節點群組已進入 v0.7 開發版：正交 fallback 與手繪／外部曲線已收斂成單一道路管線；內建道路網會把殘餘地面切成帶 `tc_block_id` 的街廓，未建築的有效基地可轉成停車場或口袋綠地。急彎上無法安全容納的基地會自動略過並在側欄警示；街廓內的面式基地切分仍在 `docs/PLAN.md` 的待辦中。以下 0.4／0.3 段落保留歷史功能介紹。
...
Python 只建立原創資產、節點樹與 UI；**修改參數與區域時，配置由 Geometry Nodes 評估**，沒有逐幀 Python handler。道路立體裁切比舊版的單面材質需要更多運算，建議先使用 100–200 m 區域。街屋資產索引為 `(樓層 - 2) × 6 + 立面變體`（0–35），鐵皮屋為 36–41，曲線道路近似直角路口用的轉角雙立面建築為 42–53（`(樓層 - 2) × 2 + 鏡像方向`）；五組集合（Buildings、Signs、Roofs、Street life、Additions）的名稱排序和索引一一對齊。`tc_parcel_id`、`tc_floors`、`tc_asset`、`tc_is_shed` 是保留在基地點上的檢查屬性。實體網格上的 `tc_layer` 可區分道路 1、人行道 2、架空線 3、殘餘地面 4、停車場 5、口袋綠地 6；殘餘地面另有 `tc_block_id`，`tc_span_id` 則辨認線路跨段。
