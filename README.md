...
目前 checkout 的城市節點群組已進入 v0.7 開發版：正交 fallback 與手繪／外部曲線已收斂成單一道路管線；內建道路網會把殘餘地面切成帶 `tc_block_id` 的街廓，未建築的有效基地可轉成停車場或口袋綠地。街廓內的面式基地切分與曲率警示仍在 `docs/PLAN.md` 的待辦中。以下 0.4／0.3 段落保留歷史功能介紹。
