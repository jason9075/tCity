...
目前 checkout 的城市節點群組已進入 v0.7 開發版：正交 fallback 與手繪／外部曲線已收斂成單一道路管線；內建道路網會把殘餘地面切成帶 `tc_block_id` 的街廓。一般基地直接從街廓臨路頂邊抽取 frontage，再生成顯式矩形基地面、退入街廓半個 `Depth` 並繼承所屬街廓；建築與空地由面中心生成。`Parcel Guides` 會以 8 × 8 細分面近似裁切到所屬街廓，並以最近基地所有權移除重疊 cell，顯示窄街廓與斜邊的不規則輪廓。複合路口布林若沒有街廓頂面會自動回退中心線排；急彎上無法安全容納的基地會自動略過並在側欄警示。精確梯形／任意多邊形地籍仍在 `docs/PLAN.md` 的待辦中。以下 0.4／0.3 段落保留歷史功能介紹。
